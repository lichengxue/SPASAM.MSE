...
plot_mse_output <- function(mods,
  ...
  first_mod <- tryCatch(if (is.nsim) mods[[1]][[1]] else mods[[1]],
                        error = function(e) NULL)
  first_rep <- tryCatch(first_mod$om$rep, error = function(e) NULL)
  
  has_finite_rep_field <- function(field) {
    x <- tryCatch(first_rep[[field]], error = function(e) NULL)
    !is.null(x) && length(x) > 0L && any(is.finite(x))
  }
  
  has_dynamic_brps <- has_finite_rep_field("log_SSB_FXSPR") &&
    has_finite_rep_field("log_Fbar_XSPR")
  has_static_brps <- has_finite_rep_field("log_SSB_FXSPR_static") &&
    has_finite_rep_field("log_Fbar_XSPR_static")
  has_spr_static <- has_finite_rep_field("log_SPR_FXSPR_static")
  
  if (!has_dynamic_brps && !has_static_brps && !has_spr_static) {
    message("Reference-point outputs were not found in mods; BRP-dependent plots will be skipped.")
  }
  ...
  call_holistic_windrose_heat <- function() {
    ...
    if (!has_dynamic_brps && !has_static_brps) {
      message("Reference points not found. Skipping holistic wind-rose/heatmap plots.")
      return(invisible(NULL))
    }
  ...
  call_radar_all <- function() {
    ...
    if (!has_dynamic_brps && any(rts == 3)) {
      message("Dynamic reference points (log_SSB_FXSPR/log_Fbar_XSPR) not found. Skipping radar type 3.")
      rts <- setdiff(rts, 3)
    }
    if (length(rts) == 0L) return(invisible(NULL))
  ...
  call_holistic_bar_all <- function() {
    ...
    if (!has_dynamic_brps && any(bts %in% c(3, 4))) {
      message("Dynamic reference points (log_SSB_FXSPR/log_Fbar_XSPR) not found. Skipping holistic bar types 3 and 4.")
      bts <- setdiff(bts, c(3, 4))
    }
    if (length(bts) == 0L) return(invisible(NULL))
  ...
  for (i in seq_along(plot_calls)) {
    ...
    if (identical(fn, ".__internal__")) {
      ...
      if (nm == "Relative performance (Catch/SSB/Fbar)") {
        ...
        if (!has_dynamic_brps) {
          message("Dynamic reference points (log_SSB_FXSPR/log_Fbar_XSPR) not found. Skipping stock-status plots.")
        } else if (safe_exists("plot_status_all")) {
        ...
        if (!has_dynamic_brps) {
          message("Dynamic reference points (log_SSB_FXSPR/log_Fbar_XSPR) not found. Skipping Kobe plots.")
        } else if (safe_exists("plot_kobe_status")) {
          base_args <- list(
            mods = mods, is.nsim = is.nsim,
            main.dir = main_dir, sub.dir = output_dir,
            width = width, height = height, dpi = dpi, col.opt = col.opt,
