...
for (i in 1:3) {
  ...
  info <- generate_basic_info(
    onto_move = onto.move.opts[i],
    onto_move_pars = onto.move.pars.opts[[i]]
  )
  ...
  plot_list[[i]] <- data.frame(
    Age = 1:12,
    MovementRate = exp(input$data$trans_mu_base)[1, 1:12, 1, 1, 1, 1],
    Scenario = move.names[i]
  )
}

plot_df <- bind_rows(plot_list)

ggplot(plot_df, aes(x = Age, y = MovementRate)) +
...
```
![Figure 1](Example_plots/ontogenetic_plot.png){width="600"}
...
# 1. First generate basic_info with the trend included
info <- generate_basic_info(
  onto_move = 1,
  onto_move_pars = c(2, 1, 0, 0),   # increasing logistic across age
  apply_mu_trend = 1,
  trend_mu_rate = array(
    0.05,
    dim = c(2, 12, 5, 2, 1)   # n_stocks, n_ages, n_seasons, n_regions, n_regions-1
  )
)

basic_info <- info$basic_info

# 2. Then generate movement using this updated basic_info
move <- generate_move(
  basic_info = basic_info,
  move.type = 2,
  move.rate = c(0.2, 0.1),
...
data <- movement_data %>%
  pivot_longer(cols = -Years, names_to = "Age", values_to = "Move_rate") %>%
  mutate(Age = factor(Age, levels = as.character(1:12)))

# 7. Plot
p <- ggplot(data, aes(x = Years, y = Move_rate, color = Age)) +
  geom_line(linewidth = 1) +
  scale_color_discrete(name = "Age") +
  theme_bw() +
  labs(
    title = "Increasing trend on mean movement",
    x = "Years",
    y = "Movement rate"
  )
