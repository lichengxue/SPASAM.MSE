...
This vignette demonstrates how to use the `SPASAM.MSE` (Woods Hole Assessment Model Management Strategy Evaluation) package to reduce data availability in the assessment (e.g., deal with data limited situations in the future) in a management strategy evaluation (MSE). **Note:** all code shown here is for demonstration purposes only; the results are for illustrative use and should not be interpreted as reflecting real-world stock assessments.
