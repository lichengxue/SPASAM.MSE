...
This vignette demonstrates an end-to-end `SPASAM.MSE` workflow that includes an
...
legend("topleft",
       ...
       col = c("blue", "red"), lwd = 2, bty = "n")
```
![Figure 1](Example_plots/ecov_plot1.png){width="600"}
...
legend("topleft",
       ...
       col = c("blue", "red"), lwd = 2, bty = "n")
```
![Figure 2](Example_plots/ecov_plot2.png){width="600"}
![Figure 3](Example_plots/ecov_plot3.png){width="600"}
...
df <- data.frame(
  Year = yrs,
  SSB_Ecov = mod1$om$rep$SSB[20:23],
  SSB_NoEcov = mod2$om$rep$SSB[20:23],
  Catch_Ecov = mod1$om$rep$pred_catch[20:23],
  Catch_NoEcov = mod2$om$rep$pred_catch[20:23]
)
...
# Rename model labels
plot_df$Model <- recode(plot_df$Model,
                        Ecov = "EM with Ecov",
                        NoEcov = "EM without Ecov")

# Plot
ggplot(plot_df, aes(x = Year, y = value, color = Model, linetype = Model)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2) +
  facet_wrap(~Variable, scales = "free_y") +
  scale_color_manual(values = c("EM with Ecov" = "red",
                                "EM without Ecov" = "blue")) +
  scale_linetype_manual(values = c("EM with Ecov" = "solid",
                                   "EM without Ecov" = "dashed")) +
  scale_x_continuous(breaks = yrs) +
  labs(
    x = "Year",
    y = NULL,
    title = "MSE Comparison: EM with Ecov vs. without Ecov"
  ) +
  theme_bw() +
  theme(
...
```
![Figure 4](Example_plots/ecov_plot4.png){width="600"}